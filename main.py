...
expense_categories = [c for c in valid_categories if c not in income_categories]
...
def add_transaction(df):
    ...
    while True:
        ...
        updated_df = pd.concat([df, pd.DataFrame([transaction])], ignore_index = True)
...
def sub_menu(df):
    while True:
        ...
        if option == 1:
            ...
            view_cumulative_net_balance(df)

        elif option == 4:
            view_all_time_overview(df)
...
def view_by_month(df):
    ...
    total_income = df[(df["Date"].dt.month_name() == month) & (df["Category"].isin(income_categories))]["Amount"].sum()
    total_expenses = df[(df["Date"].dt.month_name() == month) & (df["Category"].isin(expense_categories))]["Amount"].sum()
    ...
    if get_visualisation_choice() == "Yes":
        summary_without_income = summary.drop(income_categories, errors = "ignore")
...
def view_cumulative_net_balance(df):
    print(f"\n----------------------------")

    print(f"\nCumulative net balance:\n")

    total_income_per_month = df[df["Category"].isin(income_categories)].groupby(df["Date"].dt.month)["Amount"].sum()
    total_expenses_per_month = df[df["Category"].isin(expense_categories)].groupby(df["Date"].dt.month)["Amount"].sum()
...
def view_all_time_overview(df):
    ...
    total_income = df[df["Category"].isin(income_categories)]["Amount"].sum()
    total_expenses = df[df["Category"].isin(expense_categories)]["Amount"].sum()
